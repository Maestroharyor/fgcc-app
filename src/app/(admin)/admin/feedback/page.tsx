      ...
      <div className="mt-8 flex flex-col gap-4">
        {feedback.map((f) => {
          ...
          return (
            <article
              key={f.id}
              className="rounded-2xl border border-[var(--color-text-navy)]/8 bg-white p-5 shadow-[var(--shadow-card)]"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-display font-semibold text-[var(--color-text-navy)]">
                ...
                <div className="flex items-center gap-1 text-[var(--color-warm-gold-600)]">
              ...
              {f.enjoyed_most && (
                <p className="mt-3 text-sm text-[var(--color-text-navy)]/75">
              ...
              {f.improvements && (
                <p className="mt-2 text-sm text-[var(--color-text-navy)]/75">
                  <strong className="text-[var(--color-text-navy)]">
                    To improve:
                  </strong>{" "}
                  {f.improvements}
              ...
              {f.testimony && (
                <p className="mt-2 italic text-sm text-[var(--color-text-navy)]/85 border-l-2 border-[var(--color-warm-gold)]/40 pl-3">
                  “{f.testimony}”
                </p>
              )}
            </article>
...
function Avg({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-[var(--color-text-navy)]/8 bg-white p-5 shadow-[var(--shadow-card)]">
      <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--color-text-navy)]/55">
        {label}
      </div>
      <div className="mt-2 font-display text-3xl font-semibold text-[var(--color-text-navy)]">
        {value}
        <span className="text-base text-[var(--color-text-navy)]/45"> / 5</span>
      </div>
    </div>
  );
}
