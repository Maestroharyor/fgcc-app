...
export function ExportMenu({ queryString }: { queryString: string }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <ul className="absolute right-0 z-20 mt-2 w-44 rounded-xl border border-navy/8 bg-white p-1 shadow-lift">
          {formats.map((f) => (
            <li key={f.value}>
              <a
                ...
                className="block rounded-lg px-3 py-2 text-sm hover:bg-cream-100"
              >
                {f.label}
              </a>
            </li>
